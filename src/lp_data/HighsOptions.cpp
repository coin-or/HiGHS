...
bool commandLineOffChooseOnOk(const string& value) {
  ...
  HighsLogMessage(HighsMessageType::ERROR,
                  "Command line option value \"%s\" is not one of \"%s\", "
                  "\"%s\" or \"%s\"\n",
...
OptionStatus getOptionIndex(const std::string& name,
...
OptionStatus setOptionValue(const std::string& name,
...
OptionStatus setFindFeasibilityStrategyValue(OptionRecord* record,
                                             const std::string& option_value) {
  int value = 0;
  if (option_value == "approx_component")
    value = (int)FeasibilityStrategy::kComponentWise;
  else if (option_value == "exact")
    value = (int)FeasibilityStrategy::kExact;
  else if (option_value == "breakpoints")
    value = (int)FeasibilityStrategy::kComponentWiseBreakpoints;
  else if (option_value == "direct")
    value = (int)FeasibilityStrategy::kDirectSolve;
  else {
    HighsLogMessage(HighsMessageType::ERROR,
                    "feasibility component-wise value \"%s\" is not permitted: "
                    "legal values are approx_component(default), exact, "
                    "breakpoints, direct.\n",
                    option_value.c_str());
    return OptionStatus::ILLEGAL_VALUE;
  }
  return setOptionValue(((OptionRecordInt*)record)[0], value);
}
...
OptionStatus setOptionValue(const std::string& name,
  ...
  if (name == "feasibility_strategy")
    return setFindFeasibilityStrategyValue(option_records[index], value);
  ...
  if (type == HighsOptionType::BOOL) {
    ...
    return setOptionValue(((OptionRecordInt*)option_records[index])[0],
                          value_int);
  } else if (type == HighsOptionType::DOUBLE) {
    int value_int = atoi(value.c_str());
    double value_double = atof(value.c_str());
    ...
    return setOptionValue(((OptionRecordDouble*)option_records[index])[0],
                          atof(value.c_str()));
  } else {
    return setOptionValue(((OptionRecordString*)option_records[index])[0],
                          value);
  }
}
...
OptionStatus setOptionValue(OptionRecordInt& option, const int value) {
  if (value < option.lower_bound) {
    HighsLogMessage(HighsMessageType::ERROR,
                    "setOptionValue: Trying to set option \"%s\" to value %d "
                    "below lower bound of %d",
                    option.name.c_str(), value, option.lower_bound);
    return OptionStatus::ILLEGAL_VALUE;
  } else if (value > option.upper_bound) {
    HighsLogMessage(HighsMessageType::ERROR,
                    "setOptionValue: Trying to set option \"%s\" to value %d "
                    "above upper bound of %d",
...
OptionStatus setOptionValue(OptionRecordDouble& option, const double value) {
  if (value < option.lower_bound) {
    HighsLogMessage(HighsMessageType::ERROR,
                    "setOptionValue: Trying to set option \"%s\" to value %g "
                    "below lower bound of %g",
                    option.name.c_str(), value, option.lower_bound);
    return OptionStatus::ILLEGAL_VALUE;
  } else if (value > option.upper_bound) {
    HighsLogMessage(HighsMessageType::ERROR,
                    "setOptionValue: Trying to set option \"%s\" to value %g "
                    "above upper bound of %g",
                    option.name.c_str(), value, option.upper_bound);
    return OptionStatus::ILLEGAL_VALUE;
  }
  option.assignvalue(value);
...
OptionStatus setOptionValue(OptionRecordString& option,
                            const std::string value) {
...
OptionStatus getOptionValue(const std::string& name,
                            const std::vector<OptionRecord*>& option_records,
                            bool& value) {
  int index;
  OptionStatus status = getOptionIndex(name, option_records, index);
...
HighsStatus reportOptionsToFile(
...
void reportOption(FILE* file, const OptionRecordString& option,
  ...
  if (
      // file != stdout &&
      option.name == options_file_string)
    return;
  if (force_report || option.default_value != *option.value) {
    fprintf(file, "\n# %s\n", option.description.c_str());
