...
pair<double, double> Presolve::getBoundsImpliedFree(double lowInit,
  ...
  // low
  timer.timer_.start(aux_clocks[GetBoundsImpliedFreeLow]);
  ...
  timer.timer_.stop(aux_clocks[GetBoundsImpliedFreeLow]);
  // upp
  timer.timer_.start(aux_clocks[GetBoundsImpliedFreeUpp]);
  for (int kk = ARstart.at(i); kk < ARstart.at(i + 1); ++kk) {
    j = ARindex.at(kk);
    if (flagCol.at(j) && j != col) {
      // check if new bounds are precisely implied bounds from same row
  ...
  timer.timer_.stop(aux_clocks[GetBoundsImpliedFreeUpp]);
  return make_pair(low, upp);
}
...
bool Presolve::removeIfImpliedFree(int col, int i, int k) {
  ...
  timer.timer_.start(aux_clocks[GetAij]);
  double aij = getaij(i, col);
  timer.timer_.stop(aux_clocks[GetAij]);
  ...
  timer.timer_.start(aux_clocks[GetBoundsImpliedFree]);
  pair<double, double> p = getBoundsImpliedFree(low, upp, col, i, k);
  timer.timer_.stop(aux_clocks[GetBoundsImpliedFree]);
...
void Presolve::defineAuxClocks() {
  ...
  aux_clocks.push_back(highs_timer.clock_def("G Aij", "Gij"));
