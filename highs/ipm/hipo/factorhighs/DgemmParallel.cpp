...
dgemmParalleliser::dgemmParalleliser(const double* P, const double* R,
                                     double* Q, Int64 col, Int64 jb,
                                     DataCollector& data)
    : P_{P}, R_{R}, Q_{Q}, col_{col}, jb_{jb}, data_{data} {}

void dgemmParalleliser::run(Int64 start, Int64 end, double beta) const {
  callAndTime_dgemm('T', 'N', col_, end - start, jb_, -1.0, P_, jb_,
                    &R_[start * jb_], jb_, beta, &Q_[start * col_], col_,
                    data_);
}

void dgemmParallel(const double* P, const double* R, double* Q, Int64 col,
                   Int64 jb, Int64 row, Int64 nb, double beta,
                   DataCollector& data) {
  ...
  if (col >= nb / 2 && jb >= nb / 2 && row >= kBlockParallelThreshold * nb) {
    ...
    highs::parallel::for_each(
        0, row,
        [pt, beta](Int64 start, Int64 end) { pt->run(start, end, beta); },
        kBlockGrainSize * jb);
  } else {
